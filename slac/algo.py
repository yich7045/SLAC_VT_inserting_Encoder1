...
class SlacAlgorithm:
    ...
    def __init__(
        ...
        # Replay buffer.
        self.force_norm = torch.tensor(force_norm).to(device)
        self.buffer = ReplayBuffer(buffer_size, num_sequences, state_shape, tactile_shape, action_shape, device, force_norm)
    ...
    def preprocess(self, ob):
        ...
        tactile = torch.tensor(ob.tactile, dtype=torch.float64, device=self.device).float().div_(self.force_norm)
        with torch.no_grad():
    ...
    def update_latent(self, writer):
        ...
        loss_kld, loss_image, loss_reward, contact_loss, alignment_loss = self.latent.calculate_loss(state_, tactile_, action_, reward_, done_, self.force_norm)
